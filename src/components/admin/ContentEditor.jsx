...
// Emplacements prédéfinis utilisés par la page "À propos" et le footer du site.
// Ils apparaissent comme des raccourcis tant qu'ils n'ont pas encore été créés,
// pour que Luc n'ait pas à deviner un nom technique.
const PRESETS = [
  { key: "about_intro", label: "À propos — Texte d'introduction", type: "text" },
  { key: "about_montres_image", label: "À propos — Photo Montres", type: "image" },
  { key: "about_montres_texte", label: "À propos — Texte Montres", type: "text" },
  { key: "about_chaussures_image", label: "À propos — Photo Chaussures", type: "image" },
  { key: "about_chaussures_texte", label: "À propos — Texte Chaussures", type: "text" },
  { key: "about_pret_a_porter_image", label: "À propos — Photo Prêt-à-porter", type: "image" },
  { key: "about_pret_a_porter_texte", label: "À propos — Texte Prêt-à-porter", type: "text" },
  { key: "footer_texte", label: "Footer — Texte de présentation", type: "text" },
];
...
export default function ContentEditor({ items, onChanged }) {
  ...
  const missingPresets = PRESETS.filter((p) => !items.some((i) => i.key === p.key));
  ...
  return (
    <div className="space-y-3">
      <p className="text-ivory/50 font-body text-xs leading-relaxed mb-2">
        ...
        libellés de la roue "Collection Explorer", page "À propos", footer, etc.
      </p>
      ...
      {presetForm ? (
        <NewContentForm
          preset={presetForm}
          onSaved={() => { setPresetForm(null); onChanged(); }}
          onCancel={() => setPresetForm(null)}
        />
      ) : missingPresets.length > 0 ? (
        <div className="border border-gold/15 rounded-2xl p-3">
          <p className="text-ivory/50 font-body text-xs uppercase tracking-wide mb-2">
            À compléter pour "À propos" et le footer
          </p>
          <div className="flex flex-col gap-2">
            {missingPresets.map((p) => (
              <button
                ...
                className="text-left text-xs font-body text-gold border border-gold/30 rounded-lg px-3 py-2"
              >
                + {p.label}
              </button>
            ))}
...
function ContentItemEditor({ item, onChanged }) {
  ...
  const handleDelete = async () => {
    if (!confirm(`Supprimer "${item.label}" ? Cette action est irréversible.`)) return;
    setDeleting(true);
    try {
      await client.delete(`/content/${item.key}`);
      onChanged();
    } catch {
      alert("Impossible de supprimer ce contenu pour le moment.");
      setDeleting(false);
    }
  };
  ...
  return (
    <div className="bg-anthracite-card border border-gold/10 rounded-2xl p-4">
      <div className="flex items-start justify-between mb-2">
        <p className="font-heading text-ivory text-sm">{item.label}</p>
        <button
          onClick={handleDelete}
          disabled={deleting}
          aria-label="Supprimer ce contenu"
          className="p-1.5 border border-bordeaux-light/50 rounded-full disabled:opacity-40 shrink-0 ml-2"
        >
          <Trash2 size={13} className="text-bordeaux-light" />
        </button>
      </div>
      {item.type === "text" ? (
        <textarea
...
function NewContentForm({ onSaved, onCancel, preset }) {
  const [customLabel, setCustomLabel] = useState("");
  const [type, setType] = useState(preset?.type || "text");
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError("");
    const finalLabel = preset ? preset.label : customLabel;
    if (!finalLabel.trim()) return setError("Merci de donner un nom à cette zone.");
    if (type === "image" && !imageFile) return setError("Merci de choisir une image.");

    setSaving(true);
    try {
      const formData = new FormData();
      formData.append("key", preset ? preset.key : slugify(finalLabel) + "_" + Date.now().toString(36));
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-anthracite-card border border-gold/15 rounded-2xl p-4 relative">
      ...
      {preset ? (
        <p className="font-heading text-ivory text-sm mb-3 pr-6">{preset.label}</p>
      ) : (
